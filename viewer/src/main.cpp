...
void setupFileMenu(QMenuBar* myMenuBar, glShaderWindow* glWindow, QApplication *myApp)
{
    ...
    // Load new normal texture
    QAction* openNormalTextureAction = new QAction(myMenuBar->tr("&Load normal texture"), fileMenu);
    openNormalTextureAction->setStatusTip(myMenuBar->tr("&Opens a new normal texture image file"));
    glWindow->connect(openNormalTextureAction, SIGNAL(triggered()), glWindow, SLOT(openNewNormalTexture()));
    fileMenu->addAction(openNormalTextureAction);
...
int main( int argc, char* argv[] )
{
    ...
    QString normalTextureName = "earth2.jpg";
    ...
    appPath = appPath + "/models/";
#endif
    window.setWorkingDirectory(appPath, sceneName, textureName, normalTextureName, envMapName);
    window.show();
